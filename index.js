...
function create() {
  ...
  targets.forEach((target) => {
    const targetRect = this.add
      .rectangle(target.x, target.y, 100 * scaleX, 100 * scaleY, 0xcccccc)
      .setOrigin(0.5)
      .setStrokeStyle(2, 0x000000)
      .setInteractive({ dropZone: true });

    const text = this.add
      .text(target.x, target.y, target.shape, {
  ...
  shuffledShapeNames.forEach((name, index) => {
    const shape = this.add
      .image(100 * scaleX + index * 150 * scaleX, 100 * scaleY, name)
      .setInteractive();
  ...
  const congratsText = this.add
    .text(
      this.scale.width / 2,
      this.scale.height / 2,
      "Congratulations!\nYou dragged and dropped the shapes at the right place!",
      {
  ...
  this.input.on("drop", (pointer, gameObject, dropZone) => {
    ...
    if (target && gameObject.id === target.id && !target.filled) {
